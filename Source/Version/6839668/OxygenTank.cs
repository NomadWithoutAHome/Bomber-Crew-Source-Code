...
public class OxygenTank : SmoothDamageableRepairable
{
	[SerializeField]
	...
	private EffectScaler m_sparksEffectScaler;

	private float m_sparksEffect;
	...
	private void Start()
	{
		...
		SpawnSparks();
		m_sparksEffectScaler.SetScale(0f);
	}
	...
	public override float DamageGetPassthrough(float amt, DamageSource ds)
	{
		...
		if (m_health < 0f)
		{
			...
			if (!m_isBroken)
			{
				...
				WingroveRoot.Instance.PostEvent("OXYGEN_SHUTDOWN");
			}
			m_isBroken = true;
		}
	...
	private void SpawnSparks()
	{
		if (m_sparksInstance == null)
		{
			...
			m_sparksEffectScaler = m_sparksInstance.GetComponent<EffectScaler>();
		}
	}
	...
	private void Update()
	{
		...
		else if (!m_isBroken)
		{
			if (!m_isUnreliable)
			{
				...
				if (num2 > m_reliabilityRVal)
				{
					...
					WingroveRoot.Instance.PostEventGO("OXYGEN_BROKEN_RANDOM", base.gameObject);
					m_sparksEffect = 1f;
			...
			else
			{
				...
				m_sparksEffect -= Time.deltaTime;
				if (m_sparksEffect < 0f)
				{
					m_sparksEffect = 0f;
				}
				m_sparksEffectScaler.SetScale(m_sparksEffect);
		...
		{
			m_sparksEffectScaler.SetScale(1f);
		}
	}
	...
	public override void Repair()
	{
		...
		WingroveRoot.Instance.PostEvent("OXYGEN_REPAIRED");
	}
